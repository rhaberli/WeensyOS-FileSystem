...
int inode_indirect_alloc(uint32_t* f, bool alloc) {  //helper function for inode_block_walk
    int r;
    if(!*f) {
        if(!alloc) {
            return -ENOENT;
...
inode_block_walk(struct inode *ino, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	// LAB: Your code here.
	int r;
        if(filebno >= N_DIRECT + N_INDIRECT + N_DOUBLE) { //out of range
            return -EINVAL;
        }
        if(filebno < N_DIRECT) {
            if(ppdiskbno) {
        ...
        if(filebno < N_INDIRECT) {
            if((r = inode_indirect_alloc(&ino->i_indirect, alloc)) < 0) {
                return r;
            }
            if(ppdiskbno) {
        ...
        filebno -= N_INDIRECT;
        if((r = inode_indirect_alloc(&ino->i_double, alloc)) < 0) {
            return r;
        }
        uint32_t* entry_ptr = (uint32_t *) diskaddr(ino->i_double) + filebno/BLKSIZE;
        if((r = inode_indirect_alloc(entry_ptr, alloc)) < 0) {
            return r;
        }
        if(ppdiskbno) {
            *ppdiskbno = (uint32_t *) diskaddr(*entry_ptr) + filebno % BLKSIZE;
        } 
        return 0;
}
...
inode_get_block(struct inode *ino, uint32_t filebno, char **blk)
{
	...
	int r;
        uint32_t* ppdiskbno;
        if((r = inode_block_walk(ino, filebno, &ppdiskbno, true)) < 0) { // if block exists while 'walking'.
            return r;
        }
        if(*ppdiskbno == 0) {
            if((r = alloc_block()) < 0 ) {
...
inode_truncate_blocks(struct inode *ino, uint32_t newsize)
{
	...
	uint32_t bno, old_nblocks, new_nblocks;
        uint32_t* ptr;
        old_nblocks = (ino->i_size + BLKSIZE - 1)/BLKSIZE; //blocks that are no longer needed.
        new_nblocks = (newsize + BLKSIZE - 1)/BLKSIZE; //needs space for new blocks for file system.
        for(bno = new_nblocks; bno < old_nblocks; bno++) {
            if((r = inode_free_block(ino, bno)) < 0) {
                fprintf(stderr, "inode_truncate_blocks: inode_free_block error\n");
            }
        }
        if(ino->i_double) {
            uint32_t from = 0;
            if(new_nblocks > N_DIRECT + N_INDIRECT) {
                from = (new_nblocks - N_DIRECT - N_INDIRECT + BLKSIZE - 1)/BLKSIZE;
            }
            uint32_t to = (old_nblocks - N_DIRECT - N_INDIRECT + BLKSIZE - 1)/BLKSIZE;
            ptr = diskaddr(ino->i_double);
        ...
        if(ino->i_indirect && new_nblocks <= N_DIRECT) {
...
inode_unlink(const char *path)
{
	int r; 
        ...
        struct dirent* d;
        struct inode* pino;
        if((r = walk_path(path, &dir, &pino, &d, 0)) < 0) { //walks path in directory to unlink inodes.
            return r;
        }
        pino->i_nlink--;
        if(pino->i_nlink == 0) {
...
{
	char name[NAME_MAX]; //name of location.
        int r;
        struct inode *dir;
        struct dirent *dsrc, *ddest;
        struct inode* pino;
        if((r = walk_path(srcpath, 0, &pino, &dsrc, 0)) < 0) {
            return r;
        }
        if((r = walk_path(dstpath, &dir, NULL, NULL, name)) == 0) { //if file exists in dstpath. 
            return -EEXIST;
        }
        if(r != -ENOENT || dir == 0) {
            return r;
        }
        if((r = dir_alloc_dirent(dir, &ddest)) < 0) {
            return r;
        }
        strcpy(ddest->d_name, name);
        ddest->d_inum = dsrc->d_inum;
        pino->i_nlink++;
        inode_flush(dir);
        return 0;
}
