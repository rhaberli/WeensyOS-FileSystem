...
void kernel(const char* command) {
    ...
    res = virtual_memory_map(kernel_pagetable,
                            0,
                            0, 
                            0x100000,
                            PTE_P|PTE_W,
...
x86_64_pagetable* allocator_owner(int8_t owner) {
...
x86_64_pagetable* allocator() {
    return allocator_owner(global_process_id);
}

x86_64_pagetable* copy_page_table(x86_64_pagetable* pt0, int8_t owner) {
    int save = global_process_id;
    global_process_id = owner;
    x86_64_pagetable* pt = allocator_owner(owner);
    memset(pt, 0, PAGESIZE);
    for(int i = 0; i < PROC_START_ADDR; i += PAGESIZE) {
        vamapping vam = virtual_memory_lookup(pt0, i);
        virtual_memory_map(pt, i, vam.pa, PAGESIZE, vam.perm, allocator);
    }
    global_process_id = save;
    return pt;    
}
...
pid_t fork(void) {
    for(int8_t child = 1; child < NPROC; child++) {
        if(processes[child].p_state == P_FREE) {
            ...
            global_process_id = child;
            processes[child].p_pagetable = copy_page_table(current->p_pagetable, child);
            if(processes[child].p_pagetable == NULL) {
                return -1;
            }
            for(uintptr_t va = 0; va < MEMSIZE_VIRTUAL; va += PAGESIZE) {
                vamapping vam = virtual_memory_lookup(current->p_pagetable, va);
                log_printf("%d %d\n", va, pageinfo[vam.pn].owner);
                if(vam.perm == (PTE_W|PTE_P|PTE_U) && va != 0xB8000) {
                    x86_64_pagetable* pa = allocator();
                    if(pa == 0) {
                        panic("Ran out of memory allocation for child process %d!\n", child);
                        return -1;
                    }
                    memcpy(pa, (void *)vam.pa, PAGESIZE);
                    virtual_memory_map(processes[child].p_pagetable, va,(uintptr_t) pa, PAGESIZE, vam.perm, allocator);
...
void process_setup(pid_t pid, int program_number) {
    global_process_id = pid;
    process_init(&processes[pid], 0);
    processes[pid].p_pagetable = copy_page_table(kernel_pagetable, pid);
...
void exception(x86_64_registers* reg) {
    ...
    case INT_SYS_PAGE_ALLOC: {
        global_process_id = current->p_pid;
